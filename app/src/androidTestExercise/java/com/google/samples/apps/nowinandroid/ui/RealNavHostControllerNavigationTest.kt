...
class RealNavHostControllerNavigationTest {
    ...
    fun `Interestsタブをタップすると、TopAppBarにInterestsと表示されること`() {
        // TODO: 「`Savedタブをタップすると、TopAppBarにSavedと表示されること`()」を参考に実装する
    }
    ...
    fun `Interestsタブの詳細画面から別タブに遷移し、Interestsタブに戻ると、詳細画面が表示されたままになっていること`() {
        composeTestRule.apply {
            // TODO:
            //   1. 「`Interests画面で個々のトピックをタップするとトピック詳細画面になっていること`()」
            //       と同じ方法でトピック調査委画面に遷移する
            //   2. ボトムナビゲーション(`NiaBottomBar`テストタグ)内で`"For you"`と表示されているタブをタップする
            //   3. ボトムナビゲーション(`NiaBottomBar`テストタグ)内で`"Interests"`と表示されているタブをタップする
            //   4. 下記のassertが成功するように、プロダクトコード側に`news:expandedCard`をつける
            //      修正が必要なコード: com.google.samples.apps.nowinandroid.core.ui.NewsResourceCardKt.NewsResourceCardExpanded
