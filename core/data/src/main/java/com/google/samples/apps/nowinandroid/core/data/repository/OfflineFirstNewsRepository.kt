...
class OfflineFirstNewsRepository @Inject constructor(
    ...
    override suspend fun pullNewsResources(ids: List<String>?) {
        val networkNewsResources: List<NetworkNewsResource> = network.getNewsResources(ids = ids)
        // networkNewsResourceから参照されているトピックIDについて、存在しないものはtopicsテーブルに空のレコードを作っておく
        ...
        // networkNewsResourceに格納されている情報でnews_resourcesテーブルのレコードを更新する
        newsResourceDao.upsertNewsResources(
            newsResourceEntities = networkNewsResources
                .map(NetworkNewsResource::asEntity)
        )

        // topicとnews_resourceの関連テーブルを更新する
