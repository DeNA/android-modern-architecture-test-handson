...
class OfflineFirstNewsRepositoryMockDaoTest {
    ...
    // 練習1: RepositoryのgetNewsResources(setOf(TOPIC_1.id))を呼び出したときに、
    //     newsResourceDao.getNewsResources(setOf(TOPIC_1.id))を元に作られたデータが返されること
    @Test
    fun `getNewsResources(filterTopicIds)_DAOのgetNewsResources(filterTopicIds)由来のものが取得できること`() =
        runTest {
            ...
            // TODO テストコードを実装する。NEWS_RESOURCE_2をNewsResourceに詰め替えたものと一致することをassertする
        }

    // 練習2: ネットワークから取得したデータを元に変換したものが、DAOのupsertNewsResources()の引数に渡されることを確認するテストを書く
    @Test
    fun `pullNewsResources_ネットワークから取得したものがnewsResourceDaoのupsertNewsResourcesに渡されていること`() =
        runTest {
            ...
            // TODO newsResourceDao.upsertNewsResources()が期待通りの引数で呼び出されていることを確認する
        }
