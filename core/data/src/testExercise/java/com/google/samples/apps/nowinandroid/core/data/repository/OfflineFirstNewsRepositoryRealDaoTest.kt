...
class OfflineFirstNewsRepositoryRealDaoTest {
    ...
    // 練習1: リポジトリのgetNewsResources(setOf(TOPIC_1.id))を呼び出したときに、
    //     DBに格納されたデータのうち、ID=1のトピックに関連しているNewsResourceのみが返されることを確認するテストを書く
    @Test
    fun `getNewsResources(filterTopicIds)_DAOに格納されているレコードのうち、引数に指定したトピックIDに対応するNewsResourceだけが取れること`() =
        runTest {
            ...
            // TODO assertするコードを実装する
        }

    // 練習2: NewsResourceから参照されていないトピックのIDを指定してgetNewsResources(filterTopicIds)を呼び出すと、
    //     空の結果が返ってくることを確認するテストを書いてください
    @Test
    fun `getNewsResources(filterTopicIds)_どのNewsResourceからも参照されていないTopic IDを引数に指定すると空の結果となること`() =
        runTest {
            ...
            // TODO テストコードを実装する。
            //  リポジトリのgetNewsResourcesを呼び出すコードと、その結果が空になることをassertするコードを書く
            //  ヒント: どこからも参照されていないTopic IDは4
        }

    // 練習3: DBに格納されているトピックと、ネットワークから取得したデータから参照されているトピックが、
    //     マージされて格納されていることを確認するテストを書いてください
    @Test
    fun `pullNewsResources_DBに存在しなかったTopicが生成されていること`() = runTest {
        ...
        // TODO actualTopicIds が、
        //  「DAOに投入したデータのトピックID」と「ネットワークから取ってきたデータから参照されたトピックID」
        //  をマージしたものになっていることをassertするコードを実装する。
    }
