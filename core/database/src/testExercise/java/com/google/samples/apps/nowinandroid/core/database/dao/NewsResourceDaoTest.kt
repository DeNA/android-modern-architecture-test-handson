...
class NewsResourceDaoTest {
    ...
    // 練習1: `upsertNewsResources`を使って格納しておいたレコードについて、`deleteNewsResources`によって削除できること
    @Test
    fun `deleteNewsResources_引数に指定したIDのNewsResourceEntityが削除されること`() = runTest {
        ...
        // TODO toDeleteを引数にしてdeleteNewsResources を呼び出し、その後に getNewsResources すると、
        //  toDelete 以外のものだけが残っていることをassertするテストを実装する。
    }

    // 練習2: 同じIDのレコードを2回 insertOrIgnoreNewsResources すると、最初に格納したレコードが残ること
    @Test
    fun `insertOrIgnoreNewsResources_格納済みのレコードと同じidのNewsResourceEntityを格納しようとすると無視されること`() =
        runTest {
            ...
            // TODO getNewsResources()を呼び、期待通りの結果になっていることをassertする実装を書く
